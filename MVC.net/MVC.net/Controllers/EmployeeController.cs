        ...
        public ActionResult AddOrEdit(Employee emp)
        {
            using (DBModels db = new DBModels())
            {
                if (emp.EmployeeID == 0)
                {
                    ...
                    db.SaveChanges();
                    return Json(new { success = true, message = "Saved Successful" }, JsonRequestBehavior.AllowGet);
                }
                else {
                    db.Entry(emp).State = System.Data.Entity.EntityState.Modified;
                    db.SaveChanges();
                    return Json(new { success = true, message = "Updated Successful" }, JsonRequestBehavior.AllowGet);
                }
            }
